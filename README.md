...
Next, I implemented **LightGCN**, which simplifies traditional graph convolutional networks by removing unnecessary non-linearities and feature transformations. Unlike Matrix Factorization, LightGCN propagates embeddings over the user-item interaction graph to capture higher-order connectivity patterns.
...
Since the dataset was large, I evaluated all models on **300 randomly sampled users** with stratification to balance highly active and sparse users. This made the results both practical and representative of real-world scenarios.
...
While **precision@k** is a common metric in recommender systems, it doesn’t account for *where* in the ranking the relevant items appear. To build a system that prioritizes placing relevant movies at the very top of the list, I chose:
...
- **LightGCN:**
  - Brought improvements over Matrix Factorization but plateaued before NGCF’s performance.

- **Enhanced NGCF:**
  - NDCG@10: ~0.6 (unstratified evaluation)
  - Outperformed both Matrix Factorization and LightGCN. The addition of attention weights and regularization helped stabilize training and improved ranking quality.
